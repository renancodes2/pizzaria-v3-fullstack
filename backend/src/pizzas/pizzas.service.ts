...
import { CreatePizzaDto, TranslationDto } from './dto/create-pizzas.dto';
import { UpdatePizzaDto } from './dto/update-pizzas.dto';
import { FilterPizzaDto } from './dto/filter-pizzas.dto';
...
export class PizzaService {
  ...
  async create(data: CreatePizzaDto) {
    ...
    if (existingPizza) {
      throw new BadRequestException('Slug is already in use by another pizza');
    }

  const { translations, price, imageUrl, ...rest } = data;
    ...
    return this.prisma.pizza.create({ data: payload, include: { translations: true } });
  }
  ...
  const { translations, price, slug, imageUrl, ...rest } = data;
    ...
    return this.prisma.pizza.update({ where: { id }, data: updatePayload, include: { translations: true } });
  }
